...
func BenchmarkTrieIndexGlob4(b *testing.B) {
	f, _, _ := trieServer.expandGlobs("service-2[3-4].server-02[1-9].metric-namespace-0[2-3]0.cpu")
	b.Logf("len(f) = %+v\n", len(f))
	// fmt.Println(f)
	for i := 0; i < b.N; i++ {
		trieServer.expandGlobs("service-2[3-4].server-02[1-9].metric-namespace-0[2-3]0.cpu")
	}
}
